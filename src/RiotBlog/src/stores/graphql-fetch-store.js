 ...
 */
// GraphQLFetchStore definition.
// Flux stores house application logic and state that relate to a specific domain.
// In this case, a list of todo items.
import RiotControl from 'riotcontrol';

const baseGraphQLApi = 'http://localhost:31949/api/GraphQL/';

function GraphQLFetchStore() {
	...
	self.on('graphql_execute', function(query) {
        console.log('graphql_execute:');
        var url = self.baseGraphQLApi;
        RiotControl.trigger(
            'fetch',
            url,
            {
            ...
            {name:'graphql_execute_ack'});
    })

    self.on('graphql_fetch', function() {
